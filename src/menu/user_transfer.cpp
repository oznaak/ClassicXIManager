...
static void UpsertIncomingLoanInterestMessage(int managerId, int userClubId,
                                              int playerId,
  ...
  {
    std::stringstream q;
    q << "SELECT rp.name FROM loan_deals ld"
      << " JOIN teams rp ON rp.id=ld.receiving_club_id"
      << " WHERE ld.manager_id=" << managerId
      << " AND ld.player_id=" << playerId
      << " AND ld.loaning_club_id=" << userClubId
      << " AND ld.direction='incoming_loan_out'"
      << " AND ld.initiating_club_id!=" << userClubId
      << " AND ld.created_date='" << UTSql(currentDate) << "'"
      << " AND ld.status IN ('offered','negotiating')"
      << " GROUP BY rp.id,rp.name"
      << " ORDER BY MAX(ld.loan_fee) DESC, rp.name ASC;";
    DatabaseResult *r = GetDB()->Query(q.str().c_str());
    if (r) {
      for (unsigned int i = 0; i < r->data.size(); i++)
        clubs.push_back(UTCell(r,i,0));
      delete r;
    }
  }
  if (clubs.empty()) return;

  std::string subject = clubs.size() == 1
    ? "Incoming loan offer: " + pname
    : std::to_string((int)clubs.size()) + " clubs interested in loaning " + pname;
  std::string body = clubs.size() == 1
    ? clubs[0] + " want to take " + pname + " on loan. Review it in Transfers."
    : std::to_string((int)clubs.size()) + " clubs are interested in loaning " + pname + ":\n";
  if (clubs.size() > 1) {
    for (const std::string &club : clubs) body += "- " + club + "\n";
    body += "Review them in Transfers > Incoming Loans.";
  }

  std::vector<int> inboxIds;
  {
    std::stringstream q;
    q << "SELECT id FROM manager_inbox"
      << " WHERE manager_id=" << managerId
      << " AND game_date='" << UTSql(currentDate) << "'"
      << " AND category='transfer'"
      << " AND (subject='Incoming loan offer: " << UTSql(pname) << "'"
      << " OR subject LIKE '%clubs interested in loaning " << UTSql(pname) << "')"
      << " ORDER BY id DESC;";
    DatabaseResult *r = GetDB()->Query(q.str().c_str());
    if (r) {
      for (unsigned int i = 0; i < r->data.size(); i++)
        inboxIds.push_back(atoi(UTCell(r,i,0).c_str()));
      delete r;
    }
  }

  int inboxId = inboxIds.empty() ? 0 : inboxIds[0];
  if (inboxId > 0) {
    std::stringstream uq;
    uq << "UPDATE manager_inbox SET subject='" << UTSql(subject)
       << "',body='" << UTSql(body)
       << "',is_read=0 WHERE id=" << inboxId << ";";
    UTExec(uq.str());
    for (unsigned int i = 1; i < inboxIds.size(); i++)
      UTExec("DELETE FROM manager_inbox WHERE id=" + std::to_string(inboxIds[i]) + ";");
  } else {
    InsertInboxMessage(managerId, subject, body, "transfer", currentDate);
  }
}

static long long GetClubTransferBudgetUT(int managerId, int clubId) {
  std::stringstream q;
  q << "SELECT transfer_budget FROM club_finances"
    << " WHERE manager_id=" << managerId << " AND club_id=" << clubId << " LIMIT 1;";
  DatabaseResult *r = GetDB()->Query(q.str().c_str());
  long long budget = 0;
  if (r && r->data.size() > 0) budget = atoll(UTCell(r,0,0).c_str());
  if (r) delete r;
  return budget;
}
    ...
    if (ev.type == "galactico") {
      ...
      // Inject forced negotiation with extreme values
      {
        ...
        { std::stringstream cnq; cnq << "SELECT name FROM teams WHERE id=" << ev.clubId << ";";
          ...
          if (cnr) delete cnr; }
        if (sellerClubId == userClubId) {
          InsertInboxMessage(managerId, "Incoming bid expected: " + pname,
            cname + " are preparing a major offer for " + pname + ". Review it in Transfers.",
      ...
      // manager-scoped market table so separate careers do not inherit it.
      {
        ...
        { std::stringstream pnq; pnq << "SELECT firstname||' '||lastname, age FROM players WHERE id=" << ev.playerId << ";";
          ...
          if (pnr && pnr->data.size() > 0) pname = UTCell(pnr,0,0);
          if (pnr) delete pnr; }
        if (ev.clubId == userClubId) {
          InsertInboxMessage(managerId, "Clubs circling " + pname,
            pname + " has attracted attention after a rise in reputation.",
            "transfer", currentDate);
        } else {
          InsertTransferNews(managerId, currentDate,
...
void RespondToOffer(int managerId, int negotiationId, const std::string &action,
  ...
  if (!isUserBid) {
    if (action == "reject") {
      ...
      UTExec(uq.str());
      InsertInboxMessage(managerId, "Incoming transfer offer rejected",
        "You rejected the transfer offer for " + GetPlayerNameUT(playerId) + ".",
        "transfer", currentDate);
      return;
    }
...
static void ActivateLoanDeal(int managerId, int loanDealId,
  ...
  std::string pname = GetPlayerNameUT(playerId);
  int userClubId = GetManagedClubId(managerId);
  if (loaningClub != userClubId && receivingClub != userClubId) {
    InsertTransferNews(managerId, currentDate,
...
void InitiateLoanOffer(int managerId, int userClubId, int playerId,
  ...
  std::string pname = GetPlayerNameUT(playerId);
  bool userInitiated = (initiatingClub == userClubId);
  bool userInvolved = (loaningClubId == userClubId || receivingClubId == userClubId);
  if (userInitiated) {
    InsertInboxMessage(managerId, "Loan offer submitted: " + pname,
      "The loan proposal is now being reviewed.", "transfer", currentDate);
  } else if (userInvolved && loaningClubId == userClubId) {
    UpsertIncomingLoanInterestMessage(managerId, userClubId, playerId, currentDate);
  } else if (!userInvolved) {
    InsertTransferNews(managerId, currentDate,
      GetTeamNameUT(receivingClubId) + " make loan approach for " + pname,
      "rumour", playerId, loaningClubId, receivingClubId);
  }
}
...
static void GenerateAILoanActivity(int managerId, int userClubId,
  ...
  std::string recentTransferCutoff = UTAddDaysSigned(currentDate, -45);
  ...
  for (unsigned int ci = 0; ci < clubs->data.size(); ci++) {
    ...
    if (((dateSeed ^ (unsigned int)(clubId * 31)) % 100) > 36) continue;
    ...
    pq << "SELECT p.id, p.age, p.sofifaPotential, p.base_stat"
       ...
       << " AND NOT EXISTS (SELECT 1 FROM transfer_negotiations tn"
       << "   WHERE tn.manager_id=" << managerId
       << "   AND tn.player_id=p.id"
       << "   AND tn.buying_club_id=" << clubId
       << "   AND tn.state='completed'"
       << "   AND tn.initiated_date>='" << UTSql(recentTransferCutoff) << "')"
       << " AND (p.age<=24 OR p.base_stat<0.68)"
       << " ORDER BY (p.sofifaPotential-p.base_stat*100) DESC, p.age ASC LIMIT 2;";
    DatabaseResult *pr = GetDB()->Query(pq.str().c_str());
    if (pr && pr->data.size() > 0) for (unsigned int pi = 0; pi < pr->data.size(); pi++) {
      int pid = atoi(UTCell(pr,pi,0).c_str());
      std::stringstream iq;
  ...
  {
    std::stringstream cq;
    cq << "SELECT COUNT(*) FROM player_market_status pms"
    ...
    DatabaseResult *cr = GetDB()->Query(cq.str().c_str());
    if (cr && cr->data.size() > 0) listedCount = atoi(UTCell(cr,0,0).c_str());
    if (cr) delete cr;
  }
  if (listedCount < 18) {
    std::stringstream bq;
    bq << "SELECT p.id"
       ...
       << " AND COALESCE(pms.status,'')=''"
       << " AND (p.age<=23 OR (p.age<=25 AND p.base_stat<0.70))"
       << " AND NOT EXISTS (SELECT 1 FROM loan_deals ld"
       << "   WHERE ld.manager_id=" << managerId
       << "   AND ld.player_id=p.id"
       << "   AND ld.status IN ('offered','negotiating','accepted_pending_player','active'))"
       << " AND NOT EXISTS (SELECT 1 FROM transfer_negotiations tn"
       << "   WHERE tn.manager_id=" << managerId
       << "   AND tn.player_id=p.id"
       << "   AND tn.buying_club_id=COALESCE(pss.team_id,p.team_id)"
       << "   AND tn.state='completed'"
       << "   AND tn.initiated_date>='" << UTSql(recentTransferCutoff) << "')"
       << " ORDER BY p.sofifaPotential DESC, p.age ASC, p.base_stat ASC"
       << " LIMIT " << (18 - listedCount) << ";";
    DatabaseResult *br = GetDB()->Query(bq.str().c_str());
    if (br) {
      for (unsigned int i = 0; i < br->data.size(); i++) {
        int pid = atoi(UTCell(br,i,0).c_str());
        std::stringstream iq;
        iq << "INSERT OR REPLACE INTO player_market_status"
           << "(manager_id,player_id,status,set_date,asking_price)"
           << " VALUES(" << managerId << "," << pid << ",'loan_listed','"
  ...
  for (unsigned int bi = 0; bi < buyers->data.size(); bi++) {
    ...
    if (((dateSeed ^ (unsigned int)(buyerId * 911)) % 100) > 38) continue;

    std::stringstream tq;
    tq << "SELECT p.id, COALESCE(pss.team_id,p.team_id), p.weekly_wage,"
       << " p.playervalue, p.age, p.sofifaPotential"
       << " FROM players p"
       << " LEFT JOIN player_market_status pms"
       << " ON pms.manager_id=" << managerId << " AND pms.player_id=p.id"
       << " LEFT JOIN player_save_state pss"
       ...
       << "   AND tn.state='completed'"
       << "   AND tn.initiated_date>='" << UTSql(recentTransferCutoff) << "')"
       << " ORDER BY (CASE WHEN COALESCE(pss.team_id,p.team_id)=" << userClubId
       << " THEN 20 ELSE 0 END) + (CASE WHEN pms.status='loan_listed' THEN 10 ELSE 0 END) DESC,"
       << " p.base_stat DESC LIMIT 12;";
    DatabaseResult *tr = GetDB()->Query(tq.str().c_str());
    ...
    long long playerValue = atoll(UTCell(tr,pick,3).c_str());
    int playerAge = atoi(UTCell(tr,pick,4).c_str());
    int playerPotential = atoi(UTCell(tr,pick,5).c_str());
    delete tr;
    if (parentId <= 0 || parentId == buyerId) continue;

    int wagePct = 50 + (int)((dateSeed + buyerId + playerId) % 6) * 10;
    wagePct = std::min(100, wagePct);
    long long buyerBudget = GetClubTransferBudgetUT(managerId, buyerId);
    int loanFee = (int)std::max(0LL, wage * (long long)(wagePct / 10));
    if (buyerBudget > 0) loanFee = (int)std::min<long long>(loanFee, buyerBudget / 20);
    int optionFee = 0;
    int mandatoryFee = 0;
    std::string mandatoryTrigger;
    int mandatoryApps = 0;
    bool canAffordClause = buyerBudget > playerValue / 2;
    unsigned int clauseSeed = dateSeed ^ (unsigned int)(buyerId * 17) ^ (unsigned int)(playerId * 97);
    if (canAffordClause && playerValue > 0 && (clauseSeed % 100) < 28) {
      optionFee = (int)(playerValue * (0.85 + (double)((clauseSeed >> 3) % 21) / 100.0));
    }
    if (canAffordClause && optionFee == 0 && playerValue > 0
        && playerAge <= 24 && playerPotential >= 78 && (clauseSeed % 100) < 8) {
      mandatoryFee = (int)(playerValue * (0.80 + (double)((clauseSeed >> 5) % 16) / 100.0));
      mandatoryTrigger = ((clauseSeed >> 9) % 2) ? "appearances" : "end_date";
      mandatoryApps = mandatoryTrigger == "appearances" ? 12 + (int)((clauseSeed >> 12) % 9) : 0;
    }
    ...
    InitiateLoanOffer(managerId, userClubId, playerId, parentId, buyerId,
      loanFee, wagePct, "squad_player", 1, optionFee, optionFee > 0 ? AddDays(currentDate, 150) : "",
      mandatoryFee, mandatoryTrigger, mandatoryApps,
  ...
  for (auto &l : loans) {
    ...
    if (fit >= 52) {
      ...
      uq << "UPDATE loan_deals SET status='accepted_pending_player',"
         ...
         << " WHERE id=" << l.id << ";";
      UTExec(uq.str());
      if (l.initClub == userClubId && (l.loanClub == userClubId || l.rcvClub == userClubId)) {
        InsertInboxMessage(managerId, "Loan counter accepted: " + GetPlayerNameUT(l.playerId),
          GetTeamNameUT(l.rcvClub) + " accepted the loan terms. The loan is pending final checks.",
          "transfer", currentDate);
      }
    } else if (fit >= 38) {
      int newPct = std::min(100, l.wagePct + 20);
      int counterInitiator = (l.direction == "incoming_loan_out") ? l.rcvClub : l.loanClub;
      std::stringstream uq;
      uq << "UPDATE loan_deals SET status='negotiating',"
         ...
         << ",initiating_club_id=" << counterInitiator
         << ",created_date='" << currentDate << "',user_pending_action='review_counter'"
         << " WHERE id=" << l.id << ";";
      UTExec(uq.str());
      if (l.initClub == userClubId && (l.loanClub == userClubId || l.rcvClub == userClubId)) {
        InsertInboxMessage(managerId, "Loan counter returned: " + GetPlayerNameUT(l.playerId),
          GetTeamNameUT(l.rcvClub) + " came back with adjusted loan terms.",
          "transfer", currentDate);
      }
    } else {
      std::stringstream uq;
      ...
      UTExec(uq.str());
      if (l.initClub == userClubId && (l.loanClub == userClubId || l.rcvClub == userClubId)) {
        InsertInboxMessage(managerId, "Loan counter rejected: " + GetPlayerNameUT(l.playerId),
          GetTeamNameUT(l.rcvClub) + " rejected your loan terms.",
          "transfer", currentDate);
      }
    }
  }
...
void RespondToLoanOffer(int managerId, int loanDealId,
  ...
  if (action == "accept") {
    ...
    oq << "UPDATE loan_deals SET status='collapsed',collapse_reason='accepted_other_loan'"
       ...
       << " AND status IN ('offered','negotiating','accepted_pending_player');";
    UTExec(oq.str());
    InsertInboxMessage(managerId, "Loan offer accepted: " + GetPlayerNameUT(playerId),
      "You accepted " + GetTeamNameUT(rcvClub) + "'s loan offer. The loan is pending final checks.",
      "transfer", currentDate);
    return;
  }
  ...
  if (action == "counter") {
    ...
    uq << "UPDATE loan_deals SET status='negotiating',loan_fee=" << counterFee
       ...
       << ",option_to_buy_fee=" << std::max(0, optionFee)
       << ",option_to_buy_deadline='" << UTSql(optionDeadline) << "'"
       << ",mandatory_buy_fee=" << std::max(0, mandatoryFee)
       << ",mandatory_buy_trigger='" << UTSql(mandatoryTrigger) << "'"
       << ",mandatory_buy_appearances=" << std::max(0, mandatoryAppearances)
       << ",initiating_club_id=" << userClubId
       << ",user_pending_action='waiting_decision'"
  ...
  if (action == "recall" && loanClub == userClubId) {
    ...
    uq << "UPDATE loan_deals SET status='recalled',collapse_reason='parent_recall'"
       << " WHERE manager_id=" << managerId << " AND id=" << loanDealId << ";";
    UTExec(uq.str());
    InsertInboxMessage(managerId, "Player recalled from loan",
      GetPlayerNameUT(playerId) + " has returned to your squad.",
      "transfer", currentDate);
    return;
  }
  ...
  if (action == "exercise_option") {
    ...
    if (fee > 0 && rcvClub == userClubId) {
      ...
      InsertInboxMessage(managerId, "Loan option exercised",
        "You exercised the option to buy for " + GetPlayerNameUT(playerId) + ".",
        "transfer", currentDate);
    }
  }
}
  ...
  for (auto &d : deals) {
    ...
    if (mandatoryDue) {
      ...
      if (d.loanClub == userClub || d.rcvClub == userClub) {
        InsertInboxMessage(managerId, "Mandatory loan buy triggered",
          "The mandatory buy clause for " + GetPlayerNameUT(d.playerId) + " has been triggered.",
          "transfer", currentDate);
      } else {
    ...
    if ((d.promise == "regular_starter" || d.promise == "important")
      ...
      if (expectedApps >= 2 && d.apps + 1 < expectedApps) {
        ...
        du << "UPDATE loan_deals SET last_drama_date='" << currentDate
           << "' WHERE id=" << d.id << ";";
        UTExec(du.str());
        if (d.recallMonth > 0 && curMonth >= d.recallMonth && d.loanClub == userClub) {
          ...
          InsertInboxMessage(managerId, "Player recalled from loan",
            GetPlayerNameUT(d.playerId) + " was recalled after playing-time concerns.",
            "transfer", currentDate);
          continue;
        }
      }
    ...
    if (!d.endDate.empty() && currentDate >= d.endDate) {
      ...
      if (d.loanClub == userClub || d.rcvClub == userClub) {
        InsertInboxMessage(managerId, "Loan expired",
          GetPlayerNameUT(d.playerId) + " has returned to " + GetTeamNameUT(d.loanClub) + ".",
          "transfer", currentDate);
      } else {
