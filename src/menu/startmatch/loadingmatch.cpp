...
LoadingMatchPage::LoadingMatchPage(Gui2WindowManager *windowManager, const Gui2PageData &pageData) : Gui2Page(windowManager, pageData) {
  ...
  m_skipVisual = GetMenuTask()->ConsumeMatchSkipVisual();

  // MatchData creation is always required — the match engine reads it.
  MatchData *matchData = new MatchData(GetMenuTask()->GetTeamID(0), GetMenuTask()->GetTeamID(1));
  GetMenuTask()->SetMatchData(matchData);

  if (m_skipVisual) {
    printf("[LOADINGMATCH] Silent manager match handoff\n");
    // No splash assets — no background image, no team logos, no captions.
    // g_SilentMatchLoadingOverlay is intentionally NOT set here — the prematch lineup
    // card stays visible as the loading cover via RenderImGuiPreMatchLineup().
    this->SetFocus();
    this->Show();
    ...
    printf("[LOADINGMATCH] Showing normal visual splash\n");
...
void LoadingMatchPage::Process() {
  ...
  if (m_skipVisual) {
    printf("[LOADINGMATCH] Silent handoff -> e_MenuAction_Game\n");
    // Do NOT clear g_SilentMatchLoadingOverlay here — match is still loading.
    // GamePage::Process() will clear it once the match object is ready.
